...
def plot_iterations(iterations, target=None):
    ...
    for i, img in enumerate(iterations):
        ...
        if target is None:
            ...
            # Display the RMSE
            plt.title(str(
                i + 1) + ' (' + '{0:.2f}'.format(np.sqrt(np.mean(np.square(target - img)))) + ')')
           # print('Image ' + str(i + 1) + ', Nilai PSNR 1={:0.4f}'.format(measure.compare_psnr(img,iterations[-1],img.max()-img.min())))
            PSNR = 10 * math.log10(255**2 / mse)
            SNR = 10 * math.log10(8**2 / mse)

            print('Image ' + str(i + 1) + ',Nilai MSE={}'.format(mse))
            print('Image ' + str(i + 1) + ", Nilai PSNR: " + str(PSNR) + "(dB)")
            print('Image ' + str(i + 1) + ", Nilai SNR: " + str(SNR) + "(dB)")
            print('Image ' + str(i + 1) + ", Rasio Kompresi: 1:{}\n".format(
                (img.size/2**20/(iterations[-1].size/2**20))))
